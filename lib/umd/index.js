(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('underscore'), require('bbmn-utils'), require('bbmn-core'), require('backbone.marionette'), require('bbmn-mixins'), require('backbone'), require('jquery')) :
	typeof define === 'function' && define.amd ? define(['exports', 'underscore', 'bbmn-utils', 'bbmn-core', 'backbone.marionette', 'bbmn-mixins', 'backbone', 'jquery'], factory) :
	(factory((global.bbmn = global.bbmn || {}, global.bbmn.components = {}),global._,global.bbmn.utils,global.bbmn,global.Mn,global.bbmn.mixins,global.Backbone,global.$));
}(this, (function (exports,_,bbmnUtils,BaseClass,backbone_marionette,bbmnMixins,Backbone,$) { 'use strict';

_ = _ && _.hasOwnProperty('default') ? _['default'] : _;
var BaseClass__default = 'default' in BaseClass ? BaseClass['default'] : BaseClass;
...
var Process = BaseClass.BaseClass.extend({
	constructor: function constructor(context, name, opts) {

		BaseClass.BaseClass.apply(this, arguments);
...
var BaseApp = bbmnUtils.mix(BaseClass.BaseClass).with(BaseClass.Events, startableMixin);
...
var App = BaseApp.extend({
	...
	_buildRegion: function _buildRegion(region, options) {
		if (region == null) {
			return new BaseClass.Region(options);
		} else if (BaseClass.isClass(region, BaseClass.Region)) {
			...
			var RegionClass = region.regionClass || BaseClass.Region;
...
var Schema = BaseClass.BaseClass.extend({
	getOption: function getOption$$1() {
		return bbmnUtils.getOption.apply(undefined, [this].concat(Array.prototype.slice.call(arguments)));
	}
});
...
var PropertySchema = Schema.extend({
	...
	_getByKey: function _getByKey(key) {
		...
		return bbmnUtils.clone(hash, { functions: true });
	},
	...
	getDisplayValue: function getDisplayValue(val, model) {
		...
		options = _.extend({ model: model, display: display, type: type, property: this });
		if (display) {
			if (_.isFunction(display.transform)) {
				val = display.transform.call(model, val, options);
			} else if (type.type == 'boolean' && type.sourceValues) {
				_.some(type.valueSource, function (label, key) {
					if (bbmnUtils.toBool(key) === val) {
						val = label;
						return true;
					}
				});
			} else if (type.type == 'enum' && type.sourceValues) {
				val = bbmnUtils.getFlag(type.sourceValues, val);
			}

			if (bbmnUtils.isEmptyValue(val) && display.ifEmpty) {
				val = display.ifEmpty;
			}
		}
		return val;
...
var ClassStore = BaseClass.BaseClass.extend({
	constructor: function constructor() {
		var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
...
var modelSchemaMixin = (function (Model$$1) {
	return Model$$1.extend({
		...
		display: function display(key, options) {
...
var ExtView = bbmnUtils.mix(backbone_marionette.View).with(bbmnMixins.cssClassModifiersMixin, common);

var ExtCollectionVIew = bbmnUtils.mix(backbone_marionette.CollectionView).with(bbmnMixins.cssClassModifiersMixin, bbmnMixins.customsMixin, bbmnMixins.emptyFetchMixin, bbmnMixins.improvedIndexesMixin, common);
...
var AtomText = ExtView.extend({
	...
	hasHeader: function hasHeader() {
		return !bbmnUtils.isEmptyValue(this.getHeader());
	},
	getHeader: function getHeader() {
		return this._getText('header');
	},
	hasTopText: function hasTopText() {
		return !bbmnUtils.isEmptyValue(this.getTopText());
	},
	...
	byModel: function byModel(model, options) {
		...
		var values = _.reduce(keys, function (memo, key) {
			if (model.has(key) && !bbmnUtils.isEmptyValue(model.get(key))) {
...
var TextView = BaseClass.BackboneView.extend({
	constructor: function constructor() {
		var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

		this.options = options;
		var text = options.text,
		    _options$isHtml = options.isHtml,
		    isHtml = _options$isHtml === undefined ? true : _options$isHtml,
		    property = options.property;

		this.setValue({ text: text, isHtml: isHtml, preventRender: true });
		BaseClass.BackboneView.apply(this, arguments);
	...
	setValue: function setValue(value) {
		...
		var _opts = opts,
		    preventRender = _opts.preventRender;

		var newvalue = _.pick(opts, 'text', 'isHtml');
	...
	getValue: function getValue() {
		var text = this.text || '';
		if (!this.isHtml) text = _.escape(text);
		return text;
	},
	setNodeValue: function setNodeValue() {
		this.el.innerHTML = this.getValue();
	},
	applyPropertyValue: function applyPropertyValue() {
...
*/
var User = BaseClass.Model.extend({
	...
	constructor: function constructor(hash, opts) {
		...
		BaseClass.Model.apply(this, arguments);
	...
	refresh: function refresh() {
		...
		var promise = this._refreshing = new Promise(function (resolve) {
			if (!_this3.token.hasToken()) {
				_this3.reflectChanges({ clear: true });
				resolve();
			} else {
...
var Token = BaseClass.Model.extend({
	...
	replaceBackboneAjax: function replaceBackboneAjax() {
		...
		if (!this.hasToken()) Backbone__default.ajax = nativeAjax;else Backbone__default.ajax = function () {
...
var Stack = bbmnUtils.mix(BaseClass.BaseClass).with(BaseClass.Events).extend({
...
var FakeStore = BaseClass.BaseClass.extend({
	constructor: function constructor() {
		BaseClass.BaseClass.apply(this, arguments);
		this.store = {};
	},
...
var NotifyModel = BaseClass.Model.extend({
	constructor: function constructor() {
		BaseClass.Model.apply(this, arguments);
...
var Notifies = BaseClass.Collection.extend({
	model: NotifyModel,
...
var Notifier = bbmnMixins.nextCollectionViewMixin(ExtCollectionVIew).extend({
	initialize: function initialize() {
...
var ActionModel = BaseClass.Model.extend({
...
var instanceProperties = ['name', 'label', 'order'];

var Action = BaseClass.BaseClass.extend({
	constructor: function constructor() {
		var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	...
	getOption: function getOption$$1(key) {
		return bbmnUtils.getByPath(this.options, key);
	},
...
var Store = BaseClass__default.extend({
	constructor: function constructor() {
		var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	...
	buildActions: function buildActions() {
		...
		var options = { name: name, ctor: ctor, Action: Action$$1 };

		return _.reduce(actions, function (passed, action) {

			action = _this.buildAction(action, options);
	...
	registerActions: function registerActions(raw) {
		...
		var actions = this.buildActions(raw);
		(_actions = this.actions).push.apply(_actions, toConsumableArray(actions));
	}
});
...
var store$3 = new ClassStore({
	...
	getActions: function getActions(arg, options) {
		var _this2 = this;

		var cache = this.getStore(arg);
		if (!cache) return [];
		return _.filter(cache.actions, function (action, index) {
...
var actionableMixin = (function (Base) {
	return Base.extend({
		...
		_initializeActionableActions: function _initializeActionableActions() {
			...
			if (!store$3.isExists(this)) {
				var instance = bbmnUtils.betterResult(this, 'actions', { args: [this], default: [] });
				var inherited = [];
				var waiting = this._actionsWaitingForRegister || [];
				if (this.inheritActions) {
					...
					var protoActions = getFromPrototypes(this, 'actions', {
						...
						process: function process(actions) {
							return bbmnUtils.betterResult({ actions: actions }, 'actions', { args: [_this], default: [] });
						}
					});
				...
				var rawactions = [].concat(toConsumableArray(inherited), toConsumableArray(instance), toConsumableArray(waiting));

				store$3.initialize(this, rawactions, {
					Action: this.ActionClass,
					buildAction: function buildAction(raw) {
						return _this.buildStoreAction(raw);
					}
				});
			}
		...
		getActions: function getActions() {
			...
			var actions = store$3.getActions(this, options);
			var asModels = options.asModels;

			if (asModels) {
				return _.map(actions, function (action) {
					return action.toModel(_this2);
				});
			}
			return actions;
		},
		registerActions: function registerActions() {
			...
			if (this._actionableActionsInitialized) {
				...
				var _actionsWaitingForReg;

				this._actionsWaitingForRegister || (this._actionsWaitingForRegister = []);
				(_actionsWaitingForReg = this._actionsWaitingForRegister).push.apply(_actionsWaitingForReg, toConsumableArray(actions));
			}
		},
...
var BaseModalView = bbmnUtils.mix(ExtCollectionVIew).with(bbmnMixins.destroyViewMixin);
...
var ModalView = BaseModalView.extend({
	...
	createCloseButton: function createCloseButton() {
		...
		if (!bbmnUtils.isViewClass(Button)) {
	...
	takeOptionsView: function takeOptionsView(key) {
		...
		var view = bbmnUtils.buildViewByKey(this, key, { TextView: TextView, options: options });
...
var FooterView = ExtCollectionVIew.extend({
	...
	getResolveView: function getResolveView() {
		...
		var view = new ButtonView({
			...
			onClick: function onClick() {
				return _this4.triggerClick(true);
			}
		});
		return view;
	},
	getRejectView: function getRejectView() {
		...
		var view = new ButtonView({
			text: text,
			onClick: function onClick() {
				return _this5.triggerClick(false);
			}
		});
		return view;
	},
...
var config = {
	...
	buildView: function buildView(options) {
		...
		var ModalView$$1 = bbmnUtils.takeFirst('ModalView', options, showOptions, this);
		var TextView = bbmnUtils.takeFirst('TextView', options, showOptions, this);
		var CloseButtonView$$1 = bbmnUtils.takeFirst('CloseButtonView', options, showOptions, this);
	...
	render: function render(view, stack) {
		...
		bbmnUtils.renderInNode(view, options);
...
function normalizeConfirmFooter() {
	...
	fopts.resolveWith = bbmnUtils.takeFirst('resolveWith', fopts, opts);
	fopts.rejectWith = bbmnUtils.takeFirst('rejectWith', fopts, opts);
	fopts.resolveText = bbmnUtils.takeFirst('resolveText', fopts, opts) || config.confirmResolveText;
	fopts.rejectText = bbmnUtils.takeFirst('rejectText', fopts, opts) || config.confirmRejectText;
	return opts;
}
...
var BaseSelector = bbmnUtils.mix(BaseClass.BaseClass).with(BaseClass.Events);
...
var Selector = BaseSelector.extend({
	constructor: function constructor() {
		...
		this.options = _.clone(options);
		bbmnUtils.mergeOptions.call(this, options, 'source', 'extractValue', 'sourceFilter');
	...
	_initializeSource: function _initializeSource() {
		if (!_.isObject(this.source)) {
			this.source = new Backbone.Collection();
			return;
		} else if (BaseClass.isCollection(this.source)) {
			return;
		}
		var models = _.map(this.source, function (value, id) {
			return { id: id, value: value };
		});
		this.source = new Backbone.Collection(models);
	...
	_createCollections: function _createCollections() {
		...
		var initialSelected = this.options.value || [];
		if (!_.isArray(initialSelected)) {
			initialSelected = [initialSelected];
		}
		var models = this.getSourceModels();
		this.all = new Backbone.Collection(models);
		...
		this.selected = new Backbone.Collection(selected);
	},
	_updateAll: function _updateAll(models) {
		if (models == null) {
	...
	selectRange: function selectRange(from, to) {
		...
		var action = function action(model) {
			var affected = _this2._select(model, actionOptions);
			if (affected) {
	...
	unselectRange: function unselectRange(from, to) {
		...
		var actionOptions = { silent: true };
		var action = function action(model) {
			var affected = _this3._unselect(model, actionOptions);
			if (affected) {
	...
	toggleRange: function toggleRange(from, to) {
		...
		var action = function action(model) {
			...
			var toggled = _this4._toggle(model, actionOptions);
	...
	getValue: function getValue() {
		...
		var results = this.selected.map(function (model) {
			return _this5.extractValue(model);
		});
		if (this.isMultiple()) {
			return results;
		} else {
...
exports.User = User;
exports.BearerToken = Token;
exports.ViewStack = Stack;
exports.store = store$1;
exports.View = ExtView;
exports.CollectionView = ExtCollectionVIew;
exports.AtomTextView = AtomText;
exports.TextView = TextView;
exports.notify = notify;
exports.notifies = notifies;
exports.Notifier = Notifier;
exports.Action = Action;
exports.ActionStore = store$3;
exports.actionableMixin = actionableMixin;
exports.action = action;
