...
var store$3 = {
	...
	getStoreByCtor: function getStoreByCtor(ctor) {
		return _.find(this.names, function (f) {
			return f.ctor === ctor;
		});
	},
	isNotInitialized: function isNotInitialized(arg) {
	...
	initialize: function initialize() {
		...
		var ActionClass = Action$$1 || this.Action;
		var ctor = _.isFunction(name) && name || undefined;
		name = this.getStoreName(name);
		...
		var options = { name: name, ctor: ctor, Action: ActionClass };
		var actionsByNames = {};
		var builded = _.reduce(actions, function (passed, action) {
		...
		this.names[name] = {
			name: name, ctor: ctor, actions: builded, actionsByNames: actionsByNames
		};
	},
	getStore: function getStore(arg) {
		if (_.isString(arg)) {
...
var actionableMixin = (function (Base) {
	return Base.extend({
		...
		_initializeActionableActions: function _initializeActionableActions() {
			...
			if (store$3.isNotInitialized(this.actionsStoreName || this.constructor)) {
		...
		getActions: function getActions() {
			...
			return store$3.getActions(this.actionsStoreName || this.constructor, options);
		},
		...
		executeAction: function executeAction(action) {
			...
			return store$3.exec.apply(store$3, [this.actionsStoreName || this.constructor, action, this].concat(toConsumableArray(rest)));
		}
