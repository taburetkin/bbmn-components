	...
	getDisplayValue: function getDisplayValue(val, model) {
		...
		if (display) {
			if (_.isFunction(display.transform)) {
				...
				var sourceValues = bbmnUtils.betterResult({ sourceValues: sourceValues }, 'sourceValues', { context: model, args: [model] });
				var result = bbmnUtils.getFlag(type.sourceValues, val);
