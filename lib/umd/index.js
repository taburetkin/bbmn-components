...
}(this, (function (exports,_,bbmnUtils,bbmnCore,backbone_marionette,bbmnMixins,Backbone,$) { 'use strict';
...
var Process = bbmnCore.BaseClass.extend({
	...
	triggerOnContext: function triggerOnContext(eventName) {
		...
		return bbmnUtils.triggerMethodOn.apply(undefined, triggerArgs);
	},
...
var Schema = bbmnUtils.mix(bbmnCore.BaseClass).with(bbmnCore.Events).extend({
	getOption: function getOption$$1() {
		return bbmnUtils.getOption.apply(undefined, [this].concat(Array.prototype.slice.call(arguments)));
	},
	triggerMethod: bbmnUtils.triggerMethod
...
var PropertySchema = Schema.extend({
	...
	_getByKey: function _getByKey(key) {
		...
		var hash = bbmnUtils.betterResult(this.schema, key, { args: [options, { property: this, model: this.modelSchema }], default: {} });
	...
	getLabel: function getLabel(model) {
		...
		return bbmnUtils.betterResult({ label: label }, 'label', { context: model || this, args: [model] });
	},
	...
	getDisplayValue: function getDisplayValue(val, model) {
		...
		if (display) {
			...
			if (_.isFunction(display.transform)) {
				...
				var sourceValues = bbmnUtils.betterResult({ sourceValues: type.sourceValues }, 'sourceValues', { context: model, args: [model] });
				var result = bbmnUtils.getFlag(sourceValues, val);
				if (result) val = result;
			}

			if (bbmnUtils.isEmptyValue(val) && display.ifEmpty && !options.ignoreIfEmpty) {
				val = bbmnUtils.betterResult(display, 'ifEmpty', { context: model, args: [model, options] });
			} else if (!bbmnUtils.isEmptyValue(val) && display.ifNotEmpty && !options.ignoreIfNotEmpty) {
	...
	resetValues: function resetValues() {
		...
		var value = bbmnUtils.betterResult(depended, 'value', { args: [dependedValue, this.modelSchema && this.modelSchema.getProperty(depended.name)] });
		if (model) {
...
var ClassStore = bbmnCore.BaseClass.extend({
	constructor: function constructor() {
		var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
...
var BaseView = bbmnUtils.mix(backbone_marionette.View).with(bbmnMixins.optionsMixin, bbmnMixins.cssClassModifiersMixin, {
...
var BaseCollectionVIew = bbmnUtils.mix(backbone_marionette.CollectionView).with(bbmnMixins.optionsMixin, bbmnMixins.cssClassModifiersMixin, bbmnMixins.customsMixin, bbmnMixins.emptyFetchMixin, bbmnMixins.improvedIndexesMixin, {
	_initializeRenderOnModelChange: _initializeRenderOnModelChange
});
...
var TextView = bbmnCore.BackboneView.extend({
	...
	constructor: function constructor() {
		...
		bbmnCore.BackboneView.apply(this, arguments);
...
*/
var User = bbmnCore.Model.extend({
	...
	constructor: function constructor(hash, opts) {
		...
		bbmnCore.Model.apply(this, arguments);
	...
	refresh: function refresh(tokenOptions) {
		...
		var promise = this._refreshing = new Promise(function (resolve) {
			...
			var currentAttrs = _.clone(_this3.attributes);

			if (!_this3.hasToken()) {
				_this3.reflectChanges(_.extend({}, tokenOptions, { clear: true }), currentAttrs);
				resolve();
			} else {

				_this3.fetch().then(function () {
					_this3.reflectChanges(tokenOptions, currentAttrs);
					resolve();
				}, function () {
					_this3.reflectChanges(_.extend({}, tokenOptions, { store: false }), currentAttrs);
					resolve();
				});
			}
		});
	...
	afterReflectChanges: function afterReflectChanges(prevAttrs, opts) {},
...
var Token = bbmnCore.Model.extend({
...
var Stack = bbmnUtils.mix(bbmnCore.BaseClass).with(bbmnCore.Events).extend({
...
var FakeStore = bbmnCore.BaseClass.extend({
	constructor: function constructor() {
		bbmnCore.BaseClass.apply(this, arguments);
		this.store = {};
	},
...
var NotifyModel = bbmnCore.Model.extend({
	constructor: function constructor() {
		bbmnCore.Model.apply(this, arguments);
...
var Notifies = bbmnCore.Collection.extend({
	model: NotifyModel,
...
var ActionModel = bbmnCore.Model.extend({
...
var Action = bbmnCore.BaseClass.extend({
	constructor: function constructor() {
		var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
...
var BaseActionStore = bbmnCore.BaseClass.extend({
	constructor: function constructor() {
		var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
...
var store$3 = new ClassStore({
	...
	buildStore: function buildStore(context) {
		...
		var store = new BaseActionStore({ name: name, ctor: ctor, buildAction: buildAction, Action: Action$$1 });
...
var initSelectorMixin = function initSelectorMixin(Base) {
	return Base.extend({
		...
		_handleChildviewToggleSelect: function _handleChildviewToggleSelect(arg1, arg2) {
			var view = bbmnUtils.isView(arg1) ? arg1 : arg2;
			var event = bbmnUtils.isView(arg1) ? arg2 : arg1;
...
var BaseSelector = bbmnUtils.mix(bbmnCore.BaseClass).with(bbmnCore.Events);
...
var Selector = BaseSelector.extend({
	...
	_initializeSource: function _initializeSource() {
		if (!_.isObject(this.source)) {
			...
			return;
		} else if (bbmnCore.isCollection(this.source)) {
			return;
		}
...
exports.initSelectorMixin = initSelectorMixin;
exports['default'] = index;
exports.ClassStore = ClassStore;
