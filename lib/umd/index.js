...
var Action = bbmnCore.BaseClass.extend({
	...
	exec: function exec(instance) {
		for (var _len = arguments.length, args = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
			args[_key - 1] = arguments[_key];
		}

		var decline = this.isNotAllowed(instance, args);
		if (decline) {
...
var actionableMixin = (function (Base) {
	return Base.extend({
		...
		getAction: function getAction(arg, options) {
			...
			var iteratee = _.isString(arg) ? { name: arg } : function (item) {
				return item === arg;
			};
		...
		executeAction: function executeAction(action) {
			this._initializeActionableActions();
			return store$3.exec.apply(store$3, [this.actionsStoreName || this.constructor, action, this].concat(Array.prototype.slice.call(arguments)));
		}
