...
var initSelectorMixin = function initSelectorMixin(Base) {
	return Base.extend({
		...
		_setupSelectorListeners: function _setupSelectorListeners(selector) {
			var _this = this;
			...
			this.on('before:add:child', function (colV, view) {
				_this.listenTo(view, 'toggle:select', _this._handleChildviewToggleSelect);
			});
		},
		_setupChildViewOptions: function _setupChildViewOptions(selector, addOptions) {
			var _this2 = this;

			if (!selector) return;

			var opts = bbmnUtils.takeFirst('childViewOptions', this.options, this);
			var selectorOptions = {
				selectable: true,
			...
			if (_.isFunction(opts)) {
				options = function options() {
					...
					var compiled = opts.call.apply(opts, [_this2].concat(args));
